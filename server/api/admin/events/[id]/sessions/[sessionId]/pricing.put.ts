import { sessionPricingUpdateSchema } from '#shared/schemas/ticketingSchema'
...
export default defineEventHandler(async (event) => {
  ...
  const result = await readValidatedBody(event, body => sessionPricingUpdateSchema.safeParse(body))
  if (!result.success) {
  ...
  const updated = await eventSessionService.updateSessionPricing(sessionId, eventId, session.venueId, result.data.pricingMode, result.data.currency, result.data.priceCents, result.data.sectionPrices)
